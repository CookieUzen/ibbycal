...
inputFile = st.sidebar.file_uploader(
    ...
    type=[
        'yaml',
        'yml'],
    help="Select your config.yaml or config.yml file to load data")
...
if st.sidebar.button(
        "Delete data", help="Remove all input data or and cancel uploaded files") or 'data' not in st.session_state:
...
try:
    ...
    numClasses = st.number_input(
        "Number of Classes",
        max_value=100,
        min_value=1,
        value=numClasses,
        help="How many different classes do you have?")
except KeyError:
    ...
    with st.form(key="classes"):
        if numClasses > len(st.session_state.data['classes']):
            for i in range(numClasses - len(st.session_state.data['classes'])):
                st.session_state.data['classes'].append(
                    {'name': '', 'classroom': '', 'teacher': ''})
        elif numClasses < len(st.session_state.data['classes']):
            for i in range(numClasses - len(st.session_state.data['classes'])):
                st.session_state.data['classes'].pop(
                    len(st.session_state.data['classes'] - i - 1))

        for i in range(numClasses):
            st.subheader("Class " + str(i + 1))

            st.session_state.data['classes'][i]['name'] = st.text_input(
                "Class Name (Required)", st.session_state.data['classes'][i]['name'], key=i)
            st.session_state.data['classes'][i]['classroom'] = st.text_input(
                "Class Room", st.session_state.data['classes'][i]['classroom'], key=i)
            st.session_state.data['classes'][i]['teacher'] = st.text_input(
                "Class Teacher", st.session_state.data['classes'][i]['teacher'], key=i)
...
try:
    ...
    numCycle = st.number_input(
        "Number of Days in timetable",
        min_value=1,
        max_value=100,
        value=numCycle,
        help="How many days are in your timetable?")
except KeyError:
    ...
    with st.form(key="timetable"):
        ...
        if numCycle < actualNumCycle:
            for i in range(actualNumCycle - numCycle):
                st.session_state.data['timetable'].pop(
                    len(st.session_state.data['timetable']) - 1)

        for i in range(numCycle):
            st.session_state.data['timetable'][i] = st.multiselect(
                f"Timetable for day {i+1}",
                default=st.session_state.data['timetable'][i],
                options=listOfNames,
                key=i)
...
if st.checkbox("Show weekends", help="Show and Edit weekends"):
    try:
        st.session_state.data['weekend'] = st.multiselect(
            "Choose weekend", default=st.session_state.data['weekend'], options=(
...
st.sidebar.download_button(
    ...
    "config.yaml",
    "application/x-yaml",
    help="Generate your config.yml or config.yaml file")
